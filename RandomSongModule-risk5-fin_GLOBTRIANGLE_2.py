...
BASE_FREQ = FPS / 2
SEMITONE_RATIO = ((((1 + 5 ** 0.5) / 2)-1)/10)+1
FRAME_INTERVAL = FPS/(3*5)
FRAME_INTERVAL2 =  FPS/(9*2.5)
BUFFER_OVERLAP = 0
start = True
thread_running = True
BIT_DEPTH = 16
...
def generate_triangle_wave(num_samples, frequency, amplitude=1.0):
    """Generates a triangle wave with the given frequency and amplitude."""
    # Calculate the number of periods in the signal
    num_periods = int(num_samples // (2 * frequency))

    # Generate a single period of the triangle wave
    t = np.concatenate([np.linspace(-1, 1, int(frequency)), np.linspace(1, -1, int(frequency))])

    # Generate the full triangle wave by repeating the single period
    triangle_wave = np.tile(t, num_periods)

    # Apply the amplitude scaling
    triangle_wave *= amplitude

    return triangle_wave[:num_samples]
