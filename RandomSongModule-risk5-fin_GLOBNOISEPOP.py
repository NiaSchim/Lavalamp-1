...
FRAME_INTERVAL = FPS/math.pi/2
FRAME_INTERVAL2 =  FPS/(math.sqrt(2)*2)/6
...
def generate_pink_noise(num_samples, frequency, amplitude):
    """Generates pink noise with the given frequency and amplitude."""
    # Calculate the number of samples per period
    sample_rate = SAMPLE_RATE
    samples_per_period = int(sample_rate / frequency)

    # Check if the number of samples per period is at least 2
    if samples_per_period < 2:
        raise ValueError("Frequency is too high for the given sample rate.")

    # Calculate the number of periods
    num_periods = int(math.ceil(num_samples / samples_per_period))

    # Generate the pink noise using the desired number of periods
    pink_noise = []
    for i in range(num_periods):
        period = generate_periodic_pink_noise(samples_per_period, amplitude)
        pink_noise.extend(period)

    # Truncate the pink noise to the desired length
    pink_noise = np.array(pink_noise[:num_samples])

    return pink_noise

def generate_periodic_pink_noise(num_samples, amplitude):
    """Generates periodic pink noise with the given number of samples and amplitude."""
    ...
    alpha = amplitude * (math.sqrt(2) / 10)

    # Apply a simple approximation filter to the pink noise
    for i in range(1, num_samples):
        pink_noise[i] = alpha * pink_noise[i] + (1 - alpha) * pink_noise[i - 1]

    # Normalize the pink noise to the desired amplitude
    pink_noise = (amplitude / np.max(np.abs(pink_noise))) * pink_noise

    return pink_noise

def generate_white_noise(num_samples, amplitude):
    """Generates white noise with the given amplitude and sample rate."""
    white_noise = np.random.randn(num_samples)

    # Normalize the white noise to the desired amplitude
    white_noise = (amplitude / np.max(np.abs(white_noise))) * white_noise

    return white_noise

def generate_triangle_wave(num_samples, frequency, amplitude=1.0):
    """Generates white noise minus pink noise with the given frequency and amplitude."""
    white_noise = generate_white_noise(num_samples, amplitude)
    pink_noise = generate_pink_noise(num_samples, frequency, amplitude)
    triangle_wave = white_noise - pink_noise
